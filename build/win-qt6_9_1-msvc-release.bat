...
set "BUILD_DIR=build-win-qt6_9_1-msvc-release"
...
windeployqt --release atc.exe
if errorlevel 1 exit /b %ERRORLEVEL%

if /i not "%STATIC_BUILD%"=="true" (
    windeployqt --release qcustomplot.dll
    if errorlevel 1 exit /b %ERRORLEVEL%
)

rem copy "%QT_PATH%\bin\libc++.dll" .
rem if errorlevel 1 exit /b %ERRORLEVEL%
    
rem copy "%QT_PATH%\bin\libunwind.dll" .
rem if errorlevel 1 exit /b %ERRORLEVEL%
